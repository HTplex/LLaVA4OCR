...
def job(data_id):
    ...
    # get face/mouth images for each sample, save as npz 1000*575*100*80*3
    face_frames_batch = []
    mouth_frames_batch = []
    for idx,landmarks in tqdm(enumerate(landmarks_batch),total=len(landmarks_batch)):
        ...
        begin_frame_no,end_frame_no = int(text_batch[idx]['id'].split("_")[2])*fps,int(text_batch[idx]['id'].split("_")[3])*fps
